...
`~/.local/SDK/iiXml`이고, 플랫폼별 설치물은 `~/.local/SDK/iiXml/platforms/<platform>` 아래에 둔다.
...
3. macOS 패키지를 `~/.local/SDK/iiXml`과 `~/.local/SDK/iiXml/platforms/macos`에 설치한다.
...
7. 각 cross package를 `~/.local/SDK/iiXml/platforms/<platform>`에 설치한다.
...
- `~/.local/SDK/iiXml/lib`: iiXml 공유 라이브러리
- `~/.local/SDK/iiXml/include/iiXml`: `#include <iiXml>`용 umbrella 헤더
- `~/.local/SDK/iiXml/include/iiXml.h`: 기존 파일명 기반 umbrella 헤더
- `~/.local/SDK/iiXml/include/Src`: 모듈별 공개 헤더
- `~/.local/SDK/iiXml/lib/cmake/iiXml`: `iiXmlConfig.cmake`와 export target 파일
- `~/.local/SDK/iiXml/platforms/macos`: macOS 전용 패키지 mirror
- `~/.local/SDK/iiXml/platforms/ios`: iOS 전용 패키지
- `~/.local/SDK/iiXml/platforms/android`: Android 전용 패키지
- `~/.local/SDK/iiXml/platforms/wasm`: WASM 전용 정적 패키지
...
```cmake
list(PREPEND CMAKE_PREFIX_PATH "$ENV{HOME}/.local/SDK/iiXml")
...
`~/.local/SDK/iiXml/platforms/<platform>/lib/cmake/iiXml/iiXmlConfig.cmake`로 위임하므로,
...
소스 저장소는 `Workspace/SDK/iiXml`이다. 직접 CMake를 구성해도 기본 설치
prefix는 `~/.local/SDK/iiXml`이며, 명시한 `CMAKE_INSTALL_PREFIX`는 유지한다.
